...
void insert(struct queue *queue_,int data){
    if(queue_->rear + 1 == queue_->capacity){
...
void print(struct queue queue_){
    ...
    if(!(queue_.front ==-1 && queue_.rear ==-1) && (queue_.rear-queue_.front)>=0){
...
int deleteOne(struct queue *queue_){
    int value = -999999;
    if(queue_->front==-1 || queue_->front> queue_->rear){
